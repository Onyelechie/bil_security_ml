...
This migration:
 - ensures a sentinel edge_pc row ('unknown') exists
 - backfills existing alerts with 'unknown' where edge_pc_id IS NULL
...
def upgrade() -> None:
    ...
    # We create a sentinel `edge_pcs` row with `edge_pc_id='unknown'` and
    ...
    insert_sql = (
        ...
        "VALUES ('unknown', 'unknown', NULL, 'offline')"
    )
    conn.execute(sa.text(insert_sql))

    # Backfill alerts where edge_pc_id is null
    update_sql = "UPDATE alerts SET edge_pc_id = 'unknown' " "WHERE edge_pc_id IS NULL"
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    delete_sql = (
        "DELETE FROM edge_pcs WHERE edge_pc_id = 'unknown' AND NOT EXISTS ("
        "SELECT 1 FROM alerts WHERE edge_pc_id = 'unknown')"
    )
