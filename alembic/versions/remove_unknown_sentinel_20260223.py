"""Remove sentinel 'unknown' from edge_pcs when safe
...
`edge_pcs` row with `edge_pc_id = 'unknown'` if it is no longer referenced
...
def upgrade() -> None:
    ...
    if cnt:
        ...
        print(f"Found {cnt} alerts still referencing 'unknown'; skipping sentinel removal")
        return
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute(sa.text("INSERT OR IGNORE INTO edge_pcs (edge_pc_id, site_name, last_heartbeat, status) VALUES ('unknown', 'unknown', NULL, 'offline')"))
