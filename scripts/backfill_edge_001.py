"""Backfill and cleanup script for alerts with `edge_pc_id='edge-001'`.

This is a renamed version of the previous helper. It keeps the same
behaviour but refers to the sentinel `edge-001` and provides the same
CLI surface (dry-run, mapping CSV, assign-default, cleanup-sentinel).
...
def apply_mappings(
    ...
    for (site, camera), edge in mapping.items():
        # Count to report
        select_sql = (
            "SELECT COUNT(*) as c FROM alerts WHERE edge_pc_id = 'edge-001' "
...
def main() -> None:
    ...
    p.add_argument(
        "--assign-default",
        type=str,
        help="Assign this edge_pc_id to all sentinel alerts",
    )
    p.add_argument(
        "--cleanup-sentinel",
        action="store_true",
        help="Remove sentinel 'edge-001' from edge_pcs if unreferenced",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
