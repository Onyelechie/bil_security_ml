...
def test_cleanup_older_than_removes_only_expired_files():
    with repo_temp_dir("image-storage-") as temp_dir:
        storage = ImageStorageService(str(temp_dir))
        old_path = Path(
...
def test_cleanup_older_than_requires_positive_hours():
    with repo_temp_dir("image-storage-") as temp_dir:
        storage = ImageStorageService(str(temp_dir))
...
def test_save_alert_image_sanitizes_filename_parts():
    with repo_temp_dir("image-storage-") as temp_path:
        storage = ImageStorageService(str(temp_path))
        saved_path = Path(
            storage.save_alert_image(
                site_id="../../site a",
                camera_id="cam\\x/../id",
        ...
        assert saved_path.parent == temp_path
